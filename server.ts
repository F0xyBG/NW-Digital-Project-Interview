...
io.on(
  ...
  (socket: Socket<ClientToServerEvents, ServerToClientEvents>) => {
    ...
    try {
      flow = JSON.parse(
        String((flowData[0] as { Json_flow: string }).Json_flow)
      );
    } catch (error) {
    ...
    const startBlock = flow.blocks.find(
      (block) => block.id === flow.startBlock
    );
    const waitAfterStart = flow.blocks.find(
      (block) => block.id === startBlock?.next
    );
    ...
    socket.on(
      'chatMessage',
      async (data: { messages: { role: string; content: string }[] }) => {
        ...
        let currentBlock = flow.blocks.find(
          (block) => block.id === session.currentBlockId
        );
        ...
        while (currentBlock) {
          if (currentBlock.type === 'message') {
            ...
            if (typeof currentBlock.message === 'string') {
              ...
              socket.emit('error', {
                error: 'Invalid message block structure',
              });
              return;
            }
            ...
            currentBlock = flow.blocks.find(
              (block) => block.id === currentBlock.next
            );
            ...
            const matchedIntent = currentBlock.intents?.find(
              (intent: { match_phrases: string[] }) =>
                intent.match_phrases.some((phrase: string) =>
            ...
            if (matchedIntent) {
              ...
              currentBlock = flow.blocks.find(
                (block) => block.id === matchedIntent.next
              );
            } else {
              ...
              try {
                ...
                const detectedIntent = await openai.chat.completions.create({
                  model: 'gpt-4.1-nano',
                  messages:
                    messagesToSend as OpenAI.Chat.ChatCompletionMessage[],
                });

                const content =
                  detectedIntent.choices[0]?.message?.content ?? 'No response';

                // Check if ChatGPT's response matches any intent
                const chatGPTMatchedIntent = currentBlock.intents?.find(
                  (intent: { match_phrases: string[] }) =>
                    intent.match_phrases.some((phrase: string) =>
